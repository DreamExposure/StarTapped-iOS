...
class GetFollowersAccountTask: NetworkTask {
    ...
    func execute() {
        ...
        Alamofire.request("https://api.startapped.com/v1/relation/get/followers", method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers)
                .responseJSON { response in
                    ...
                    case .success(_):
                        if response.result.value != nil {
                            ...
                            let status = NetworkCallStatus(failure: false, success: success, type: TaskType.FOLLOW_GET_FOLLOWERS).setCode(code: code).setBody(body: json).setMessage(message: json["message"].stringValue)
                    ...
                    case .failure(_):
                        ...
                        let status = NetworkCallStatus(failure: true, success: false, type: TaskType.FOLLOW_GET_FOLLOWERS)
