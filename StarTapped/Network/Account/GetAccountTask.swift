...
class GetAccountTask: NetworkTask {
    ...
    func execute() {
        ...
        Alamofire.request("https://api.startapped.com/v1/account/get", method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers)
                .responseJSON { response in
                    ...
                    case .success(_):
                        if response.result.value != nil {
                            ...
                            let status = NetworkCallStatus(failure: false, success: success, type: TaskType.ACCOUNT_GET_SELF).setCode(code: code).setBody(body: json).setMessage(message: json["message"].stringValue)
                    ...
                    case .failure(_):
                        ...
                        let status = NetworkCallStatus(failure: true, success: false, type: TaskType.ACCOUNT_GET_SELF)
