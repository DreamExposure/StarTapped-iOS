...
class LoginTask: NetworkTask {
    ...
    func execute() {
        ...
        Alamofire.request("https://api.startapped.com/v1/account/login", method: .post, parameters: params, encoding: JSONEncoding.default, headers: [:])
                .responseJSON { response in
                    ...
                    case .success(_):
                        if response.result.value != nil {
                            ...
                            let status = NetworkCallStatus(success: true, type: TaskType.AUTH_LOGIN).setCode(code: code).setBody(body: json).setMessage(message: json["message"].stringValue)

                            self.onComplete(status: status)
                        }
                        break
                    case .failure(_):
                        //This is a network error, NOT a 400 or 500 status code.
