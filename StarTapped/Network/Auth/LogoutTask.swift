...
class LogoutTask: NetworkTask {
    ...
    func execute() {
        ...
        AF.request("https://api.startapped.com/v1/account/logout", method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers)
                .responseJSON { response in
                    ...
                    case .success(_):
                        if response.value != nil {
                            ...
                            let status = NetworkCallStatus(failure: false, success: success, type: TaskType.AUTH_LOGOUT).setCode(code: code).setBody(body: json).setMessage(message: json["message"].stringValue)
